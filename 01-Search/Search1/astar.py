...
# A*算法
def a_star_search(src_state, dst_state):
	...
	for i in range(len(_city_info)):
		if _city_info['city1'][i] == src_state:
			# path_cost = _city_info['path_cost'][i]
	...
	while True:
		...
		if node.parent is not None:
			print('处理城市节点:%s\t父节点:%s\t预估损失为:%d' % (node.state, node.parent.state, node.evaluate_cost))
		else:
			print('处理城市节点:%s\t父节点:%s\t预估损失为:%d' % (node.state, None, node.evaluate_cost))
		...
		for i in range(len(_city_info)):  # 找到待扩展的城市node
			...
			# 只有城市为city1，才能找到该城市距离最终目的地的直线距离
			...
			print('\t孩子节点:%s 预估损失为:%d' % (child.state, child.evaluate_cost))
...
def frontier_priority_add(node):
	...
	# 在这里组织优先级队列的数据
	for i in range(size):
		if node.evaluate_cost < _frontier_priority[i].path_cost+_frontier_priority[i].line_cost:
...
# 	size = len(_frontier_priority)
# 	for i in range(size):
# 		if _frontier_priority[i].state == node.state and _frontier_priority[i].path_cost+_frontier_priority[i].line_cost > node.evaluate_cost:
...
def main():
	...
	while True:
		...
		result = a_star_search(src_city, dst_city)
